...
class Tp1TestCase(unittest.TestCase):
  ...
  def assertRun(self, inputPath, expectedPath, outputPath):
    ...
    self.assertEqual(len(expected), len(actual), f"Se esperaban {len(expected)} valores en la solucion pero se encontraron {len(actual)}")
    for index, (a, e) in enumerate(zip(actual, expected)):
      self.assertAlmostEqual(a, e, delta=0.0001, msg=f"Se esperaba {e} en la linea {index+1} pero se encontro {a}")
