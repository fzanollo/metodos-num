...
\begin{itemize}
    ...
    \item $\sum_{j=1, j \neq i}^{n}{n_{i,j} * r_j}$: Es una suma en la que cada en cada término se encuentra la cantidad de partidos disputados entre $e_i$ y $e_j$ ponderado por $r_j$. Cada término de la suma \textbf{aporta tanto como $n_{i, j}$ cuando $r_j$ tiene un valor cercano $1$} y aporta menos cuando el mismo tiene un valor cercano a $0$.
\end{itemize}
...
\subsubsection{Una estrategia avanzar en el ranking minimizando la cantidad de partidos}
...
\begin{itemize}
    ...
    \item Teniendo en cuenta el caracter transitivo mencionado previamente, jugar uniformemente con equipos que tengan bajo rating (dado que la probabilidad de ganarles es mayor) una cantidad considerable de partidos, mientras más, mejor. También se puede jugar siempre con un mismo equipo de rating bajo, pero la posibilidad de encontrar una ventaja se limita a que tan bien le vaya a ese único equipo.
\end{itemize}
